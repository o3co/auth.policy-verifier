...
- **Rules** are predicates over attributes (`verify(attrs, report?): boolean`). `verify` must be a **deterministic, side-effect-free function of `attrs`**: equal attributes give equal answers, and it must not mutate its input, perform I/O, or observe anything the engine cannot see. `verify` is handed a `ReadonlyAttributes` (`ReadonlyMap<string, unknown>`), so the "must not mutate" half is a compile error rather than a request.
- **Asynchronous rules** (`AsyncRule`, `decide(attrs, signal, report?): Promise<boolean>`, #225) are the one exception to "no I/O", and only to that clause: an out-of-process policy engine has to be asked over the network. Everything else holds — the answer is a function of `attrs` alone, the `CollectorContext` may not be retained and read, the attributes may not be mutated — and the purity conformance suite checks `decide` exactly as it checks `verify`. The I/O runs under a deadline (`EvaluateOptions.ruleTimeoutMs` — the server's `verify.ruleTimeoutMs` — with `RuleTimeoutError` on overrun, a deny) after both collects have finished, in `evaluate()`, which runs either kind and is asynchronous for that reason.

- **`report` is the one thing a rule may write to, and it is not a side effect** (#244). A rule that fronts a policy evaluator says how the evaluation behind one answer went — whether the evaluator ran, and against which policy revision — by calling the `ReportRuleEvaluation` the evaluator hands it for that one invocation. The reporter is made per call and dead after it, so nothing reaches another invocation through it; what is reported is part of the answer and falls under everything above (equal attributes, equal report — the conformance suite compares it). It is a reporter and **not a richer return value** on purpose: `{ passed, evaluation }` is an object, an object is truthy, and an evaluator that does not know about it — an older copy of core in a mixed install, a composite rule calling `verify` itself — would read every deny as a pass. An evaluator that passes no reporter gets the boolean it always got. The same holds for what a report may carry (#199): a key a newer core added reaches the evaluator only through the reporter — `report.boundDeterminingPolicies` for determining policies — because an older core refuses what it does not know, and a package upgraded ahead of the server must not turn that into a failed decision. That also keeps the key's bounds the checking core's: a rule never applies them from the copy of core its own package imports, so a release can change them without a mixed install refusing what a rule reports.
...
- **The published set is six packages**: `@o3co/auth.policy-verifier.core`, `.builtins`, `.cedar`, `.cedar-wasm`, `.server` and `@o3co/create-auth-policy-verifier`. `templates/standalone` and `tests/integration` are `private: true` and are stamped with the version but never published.
...
2. Verify the change set warrants the requested version bump (breaking change → major, feature → minor, fix → patch). While the major version is `0`, a breaking change bumps the **minor** instead (`0.12.x` → `0.13.0`), never the patch. SemVer §4 leaves 0.x open; the constraint is npm's caret range, which pins the minor — `^0.12.0` admits `0.12.x` and never `0.13.0` — so a breaking change released as a patch installs itself into every caret range. A feature has bumped the minor too (`0.6.0` added a package and nothing else), and the patches so far (`0.1.1`, `0.3.1`, `0.8.1`) carried no feature and no breaking change. Every breaking release so far has been a minor — `0.2.0`, which predates [`CHANGELOG.md`](CHANGELOG.md), and every section there with a **BREAKING** entry.
