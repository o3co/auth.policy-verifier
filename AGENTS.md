...
Every package and every source directory that is a module of its own has a `README.md` (a small directory may instead be described by its parent's README). Each README follows these rules:
...
When you change what a module does or depends on, update its README in the same PR.
