...
`category` is a closed set of those seven values, so filter by equality rather than by a regex over `err.message`. `collector` is the entry's position in `attribute.collectors` / `rule.collectors` plus its class, both fixed by configuration. `body_rejected` is a body-parser failure the deny envelope does not map to a 4xx (e.g. a request stream something in front of the router already read); `internal` is anything no pipeline attributed — a resource parser or authenticator that threw, or a collector that rejected with a string or other non-object rather than an `Error`. An unreachable JWKS is not among them: it is answered `401` and logged as `jwt_verification_unavailable`.

Nothing the router adds to these lines carries the credential, the claims or `context`: the category is an enum, the collector and rule names come from configuration, and the request id is validated as below. `err` is the error as thrown — what a collector writes into its own message is that collector's responsibility.
