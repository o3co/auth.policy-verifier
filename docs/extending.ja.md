...
- **評価する core がキーを知っている必要があります。** `determiningPolicies` を検査するのは、あなたの Rule が動く core の `evaluate()` — あなたのパッケージが持つ core ではなく server の core — です。この契約より古い core はキーを未知として拒否し、その Rule の completed な決定はすべて `500 rule_threw` になります。報告するパッケージと一緒に server を更新してください。
