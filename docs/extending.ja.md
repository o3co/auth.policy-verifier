...
- **書き込み先は自分のキーにし、予約済みキーは使わない。** `RESERVED_ATTRIBUTE_KEYS` はエンジンとその周辺パッケージが判断に使うキーの集合です。core の 5 キー（`scopes` / `permissions` / `roles` / `userId` / `clientId`。既定のサーバーでは `scopes` / `userId` / `clientId` は検証済みトークン由来）に加え、その構成がロードしたパッケージが自分で予約したキーが入ります。`requestContext` は `subject` とは信頼レベルが異なるため、両者が同じキーに着地してはなりません。`RequestContextAttributeCollector` はそのようなマッピングをコンストラクタで拒否します。自分で書く Collector も同じ集合を参照してください — この集合はパッケージのロードに応じて増えるので、core を import した時点でコピーせず、判定が必要なその場で読むこと。
- **自分の Collector が書くキーは自分で予約する。** この集合は core が保持するリストではなくレジストリです。`ATTR_*` 定数のすぐ横、モジュールスコープで `reserveAttributeKeys({ owner, keys, reason })` を呼ぶことが、自分の語彙を同じ guard の保護下に置く方法です。`@o3co/auth.policy-verifier.cedar` は `requestAction` / `requestResourceType` / `requestResourceId` / `requestResourceRaw` に対してまさにこれを行っています。モジュールスコープであることには意味があります: 構成が自分の Collector 名を書けるのは自分のモジュールを import しているからであり、どの Collector が構築されるよりも先に予約が済みます（`createApp` を呼ばないライブラリ利用でも同じです）。予約が最も効くのは、Collector がそのキーを*条件付きでしか*書かない場合です。あるリクエストで誰も書かない属性には競合相手がいないため、運用者がそこへマッピングしても衝突は起きず、そのまま判断材料になってしまいます。
...
**失敗したコレクターは運用者のログで名指しされます** (#200)。reject は decision を `500` で失敗させ、pipeline はエラーを変えずに、それがどのコレクターから来たかを記録します: `verify_internal_error` は `category: "collector_threw"` と `collector: "attribute.collectors[1] (YourCollector)"` — 設定内のエントリ位置とクラス名 — を持ち、`auth_collector_failures_total` はその名前で計上します。ライブラリとして使う場合は core の `FailureRecord` が同じ答えを返します（decision ごとに 1 つ作り、両方の collect と `evaluate` に渡す）。名前はランナーが付けるもので、エラーから読むことはありません — 自分で throw した `CollectorTimeoutError` も deny にはなりますが、名指しされるのはそのエラーが持つ `collector` ではなく自分の位置です。作者の責任になるのはエラーそのものです: `err` としてログに届くので、そのメッセージにトークン・クレーム・呼び出し元の `context` を含めないでください — サーバーが加えるフィールドはこれらを決して含みませんし、メッセージも含めるべきではありません（core 自身の deny エラーは例外です: `CollectorTimeoutError`、`RuleTimeoutError`、`AttributeConflictError` は、構築時のテキストではなく、分類されたコレクターやルールと識別子の形の属性キーから組み立て直して記録されます）。ルールの `ruleType` と `code` も同様で、識別子の形（英字で始まり、英数字・`_`・`.`・`-` が続く 64 文字以内）の場合だけ失敗イベント行に載り、それ以外は `redacted` として記録されます。
...
## answer の背後にある evaluation を報告する
...
interface DeterminingPolicies {                                      // #199 — boundDeterminingPolicies で組み立てる
...
- **`determiningPolicies` は completed の答えを決めた policy を名指します**（#199）— allow なら該当した permit、deny なら該当した forbid、どの policy も該当しなければ空のリストです。報告するのは `completed` の evaluation からだけです。`failed` の答えは Rule が fail-closed で失敗したもので、決めた policy はありません。`not_invoked` は何も評価していません — `evaluate()` はどちらの上のキーも、値がどう届いても拒否します。2 つのキーは `boundDeterminingPolicies(names)` で組み立て、結果を報告に展開してください。各名前を 1 度だけ、渡した順に残し、`isReportablePolicyId` が拒否するものと `DETERMINING_POLICIES_MAX`（32）を超えた分を落とし、それらをすべて `determiningPoliciesOmitted` に数えます — そのため戻り値が拒否されることはありません。名前は attributes だけで決まる順序で渡してください。engine の順序が安定しないならソートします。そうしないと 2 つの replica が同じ決定を別々に記録します（純粋性のスイートは 1 プロセス内の答えを比べるので、それを検出できないことがあります）。`revision: null` の横の id は revision と同じく未確認です。policy は作者が知っている名前で名指します — ファイル名や、policy が宣言した id です。engine が付けた通し番号ではありません。
- **評価する core がキーを知っている必要があります。** `determiningPolicies` を検査するのは、あなたの Rule が動く core の `evaluate()` — あなたのパッケージが持つ core ではなく server の core — です。この契約より古い core はキーを未知として拒否し、その Rule の completed な決定はすべて `500 internal_error`（ログ上の分類は `rule_threw`）になります。報告するパッケージと一緒に server を更新してください。
