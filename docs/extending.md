...
- **Write it under your own key, never core's.** `scopes`, `permissions`, `roles`, `userId`, `clientId` — core's `ATTR_*` set, exported as `RESERVED_ATTRIBUTE_KEYS` — are what the engine decides from, and under the default server the first, fourth and fifth come out of the verified token. `requestContext` carries a different trust level from `subject`, so the two must not land on one key. `RequestContextAttributeCollector` refuses such a mapping at construction; a collector you write yourself should consult the same set.
