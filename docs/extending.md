...
- **Write it under your own key, never a reserved one.** `RESERVED_ATTRIBUTE_KEYS` is what the engine and its packages decide from: core's five (`scopes`, `permissions`, `roles`, `userId`, `clientId` — under the default server the first, fourth and fifth come out of the verified token), plus whatever the packages a composition loaded reserved for themselves. `requestContext` carries a different trust level from `subject`, so the two must not land on one key. `RequestContextAttributeCollector` refuses such a mapping at construction; a collector you write yourself should consult the same set — read it at the moment you need the verdict, since it grows as packages load rather than being fixed at core's import.
- **Reserve the keys your own collector writes.** The set is a registry, not a list core keeps: `reserveAttributeKeys({ owner, keys, reason })`, called at module scope beside your `ATTR_*` constants, is what puts your vocabulary behind the same guard. `@o3co/auth.policy-verifier.cedar` does exactly that for `requestAction`, `requestResourceType`, `requestResourceId` and `requestResourceRaw`. Module scope is deliberate: a composition can only name your collectors by importing your module, so the reservation is in place before any collector is constructed, including for a library consumer that never calls `createApp`. Reserving matters most where your collector writes a key only *sometimes* — an attribute nobody else writes for a given request has no competing writer, so an operator mapping onto it would not collide, it would simply decide.

**Why a shared key is worse than an overwrite.** `AttributePipeline` merges the collectors' maps, and array-valued entries **union** rather than conflict (a scalar disagreement throws `AttributeConflictError`; two arrays simply concatenate). That is deliberate — `roles` and `permissions` are meant to be contributed to by several collectors — but it means a collector promoting caller-supplied data onto a shared key does not overwrite the deployment's value and lose an argument with it. It *extends* it. A request arriving with `context.groups = ["admin:write"]`, promoted onto `scopes`, is authorized for a scope its token never carried, and the decision, the logs and the metrics all look exactly like an issuer that granted it. The union is not the bug; writing to a shared bucket from an unverified source is, and the union is why it is silent.
...
**A collector that fails is named in the operator's log** (#200). A rejection fails the decision with a `500`, and the pipeline records which collector it came from without changing the error: `verify_internal_error` carries `category: "collector_threw"` and `collector: "attribute.collectors[1] (YourCollector)"` — the entry's position in config and your class name — and `auth_collector_failures_total` counts it under that name. A library consumer gets the same answer from core's `FailureRecord`: one per decision, handed to both collects and to `evaluate`. The name is the runner's, never your error's — a `CollectorTimeoutError` you throw yourself is still a deny, but it is named by your position, not by the `collector` it carries. What the author does own is the error itself: it reaches the log as `err`, so its message is yours to keep free of the token, the claims and the caller's `context` — the fields the server adds never carry them, and your message should not either. (Core's own deny errors are the exception: a `CollectorTimeoutError`, `RuleTimeoutError` or `AttributeConflictError` is logged rebuilt from the classified collector or rule and an identifier-shaped attribute key, never with the text it was constructed with.) The same goes for a rule's `ruleType` and `code`: they reach the failure lines only when identifier-shaped (a letter, then letters, digits, `_`, `.` or `-`, at most 64 characters) and are logged as `redacted` otherwise.
...
- The `AttributePipeline` runs the collectors concurrently, up to `verify.collectorConcurrency` at a time with the rest queued, and merges their results: array values are concatenated in collector order; a non-array value may be written once, or re-written with the identical value — two collectors writing *different* values to the same key throw `AttributeConflictError`, which denies the request (#174). Design accordingly — give each scalar key one owning collector rather than relying on ordering. Only so many run at once, and each is on a clock — see [Deadlines and cancellation](#deadlines-and-cancellation).
...
## Reporting the evaluation behind an answer
...
// A completed one may add, beside the revision (#199) — build them with boundDeterminingPolicies:
//   determiningPolicies?: string[]          — which policies determined the answer
...
- **`determiningPolicies` names the policies that determined a completed answer** (#199) — for an allow the permits that applied, for a deny the forbids that did, an empty list when no policy applied. Report it only from a `completed` evaluation; a `failed` one has none, because its answer is your rule failing closed. Build the two keys with `boundDeterminingPolicies(names)` and spread the result into the report: it keeps each name once, in the order you give, drops what `isReportablePolicyId` refuses and anything past `DETERMINING_POLICIES_MAX` (32), and counts all of that in `determiningPoliciesOmitted` — so what it returns is never refused. Give the names in an order that depends on the attributes alone: sort them if your engine's order is not stable, or two replicas record one decision two ways (the purity suite compares answers within one process, so it may not catch that). Beside `revision: null` your ids are as unconfirmed as the revision. Name a policy the way its author knows it — a file name, a policy's declared id — not by an index the engine made up.
- **The core that evaluates must know the keys.** `determiningPolicies` is checked by the `evaluate()` of the core your rule runs under — the server's, not your package's own copy. A core older than this contract refuses the keys as unknown, and every completed decision of your rule answers `500 internal_error`, logged as `rule_threw`. Upgrade the server with the package that reports them.
