...
// A completed one may add, beside the revision (#199):
//   determiningPolicies?: string[]          — which policies determined the answer
//   determiningPoliciesOmitted?: number     — how many it named past the bound
...
- **`determiningPolicies` names the policies that determined a completed answer** (#199) — for an allow the permits that applied, for a deny the forbids that did, an empty list when no policy applied. Report it only from a `completed` evaluation; a `failed` one has none, because its answer is your rule failing closed. It is a set: no id twice, in an order that depends on the attributes alone (sort it if your engine's order is not stable, or the purity suite will catch the difference). At most `DETERMINING_POLICIES_MAX` (32) ids, each 1 to `POLICY_ID_MAX_LENGTH` (256) characters with no control character; anything your engine named beyond that — past the bound, or an id you cannot fit — goes into `determiningPoliciesOmitted` as a count, never silently away. Name a policy the way its author knows it (for `packages/cedar`, the file name), not by an index the engine made up.
