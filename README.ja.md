...
- **DSL ロックインなし** — 認可ロジックは TypeScript。DSL は一切必須ではない。Cedar は「オプションの同居言語」として利用できる（[`packages/cedar`](packages/cedar/)）: 本物の Cedar evaluator が 1 ルールグループとして動き、Collector が集めた事実を entity store の構築・同期なしで判定する — in-process なら [`packages/cedar-wasm`](packages/cedar-wasm/)、out-of-process なら同じ port 越しに cedar-agent へ — deployment ごとの依存関係の選択で、実測の sizing 表は `packages/cedar` にある。どちらの言語を選んでも topology には縛られない — 同じ `.cedar` ファイルは後から埋め込み evaluator にも、（1 ファイル 1 policy の配置なら）Cedar Agent にも無変更でロードでき、[protobuf.interceptors](https://github.com/o3co/protobuf.interceptors) 経由で共通 `VerifierEndpoint` の背後をまるごと OPA / Cedar に差し替える道も残る。
...
| [`packages/cedar`](packages/cedar/) | `@o3co/auth.policy-verifier.cedar` | オプションの Cedar ポリシー評価（policy set ごとに 1 ルールグループ）。`CedarEngine` port の背後で評価し、evaluator 自体は持たない |
| [`packages/cedar-wasm`](packages/cedar-wasm/) | `@o3co/auth.policy-verifier.cedar-wasm` | in-process の Cedar engine（`@cedar-policy/cedar-wasm`）。import するだけで登録される |
...
`determiningPolicies` は Rule の提供者が名付ける policy id の集合で、どの policy も該当しなかったときは空のリストです。最大 `DETERMINING_POLICIES_MAX`（32）個、各 id は制御文字・bidi 文字を含まない well-formed な 1〜128 UTF-16 単位の文字列です（`POLICY_ID_MAX_LENGTH`、`POLICY_ID_FORBIDDEN_RANGES`）。evaluator がそれを超えて名指したもの — 上限を超えた分や、その形に収まらない id — は `determiningPoliciesOmitted` に件数として入ります（0 でないときだけ現れます）。`"revision": null` の横にある id は revision と同じく未確認で、evaluator が保持していた何らかの set の policy を指します。`failed` と `not_invoked` の evaluation はどちらのキーも持ちません。その答えはどの policy も決めていないからです。知らない Rule も持ちません — **無い = Rule が言わなかった**。deny の `code` は変わりません。id はその横に並ぶもので、置き換えるものではありません。
