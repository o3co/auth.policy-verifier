...
- **DSL ロックインなし** — 認可ロジックは TypeScript。DSL は一切必須ではない。Cedar は「オプションの同居言語」として利用できる（[`packages/cedar`](packages/cedar/)）: 本物の Cedar evaluator が 1 ルールグループとして動き、Collector が集めた事実を entity store の構築・同期なしで判定する — in-process なら [`packages/cedar-wasm`](packages/cedar-wasm/)、それ以外でも同じ port に登録した engine で。どちらの言語を選んでも topology には縛られない — 同じ `.cedar` ファイルは後から埋め込み evaluator にも Cedar Agent にも無変更でロードでき、[protobuf.interceptors](https://github.com/o3co/protobuf.interceptors) 経由で共通 `VerifierEndpoint` の背後をまるごと OPA / Cedar に差し替える道も残る。
