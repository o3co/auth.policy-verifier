...
- [`src/types.mts`](src/types.mts) — 判定の契約。
  ...
  - `ReportRuleEvaluation` と `RuleEvaluation`（および `RuleEvaluationStatus`） — ポリシー評価器を前段に持つルールが、1 つの答えの背後にある評価を報告する手段: 評価器が走ったか、どのポリシーリビジョンに対してか（`null` は明示的な「不明」）、completed なら答えを決めたポリシーはどれか（#199）。`evaluate()` は呼び出しごとに reporter を 1 つ作り、報告されたリビジョンは `POLICY_REVISION_PATTERN` と `POLICY_REVISION_MAX_LENGTH` に、決めたポリシーは `DETERMINING_POLICIES_MAX`、`POLICY_ID_MAX_LENGTH`、`POLICY_ID_FORBIDDEN_RANGES` に照らして検査されます。この検査は `isReportablePolicyId` そのもので、`boundDeterminingPolicies` はそれを通る報告を組み立てます。[docs/extending.ja.md](../../docs/extending.ja.md#answer-の背後にある-evaluation-を報告する) を参照。
  - `Role` — ロール名とそのパーミッション。
