...
| `Rule` | `{ ruleType: string; code: string; message: string; verify(attrs: ReadonlyAttributes, report?: ReportRuleEvaluation): boolean }` — `verify` は `attrs` の決定的かつ副作用のない関数でなければならない。[AGENTS.md — Collector / Rule / Attribute Contract](../../AGENTS.md#collector--rule--attribute-contract) を参照 |
| `AsyncRule` | `{ ruleType: string; code: string; message: string; readonly async: true; decide(attrs: ReadonlyAttributes, signal: AbortSignal, report?: ReportRuleEvaluation): Promise<boolean> }` — `Rule` と同じ契約で I/O を行うもの、デッドラインのもとで実行される (#225)。`isAsyncRule` は判別子 `async` を読む |
...
| `ReportRuleEvaluation` | `(evaluation: RuleEvaluation) => void` — policy evaluator を背後に持つ Rule が、1 つの answer の背後にある evaluation を報告する手段。`evaluate()` が呼び出しごとに 1 つ作って `verify` / `decide` に渡す。Rule が返すのは boolean のままなので、reporter を渡さない evaluator（mixed install の中の古い core）は deny を deny として読む。[docs/extending.ja.md](../../docs/extending.ja.md#answer-の背後にある-evaluation-を報告する) を参照 |
