...
- One reporter per invocation: at most one report, checked and frozen onto that outcome
  ...
  (`isReportablePolicyId`) is what the reporter's `boundDeterminingPolicies` filters with. Every
  reporter carries that function, neither writable nor configurable, so the bounds a rule
  applies are those of the core that checks its report —
