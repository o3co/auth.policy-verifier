...
| `Rule` | `{ ruleType: string; code: string; message: string; verify(attrs: ReadonlyAttributes): RuleAnswer }` — `verify` must be a deterministic, side-effect-free function of `attrs`. See [AGENTS.md — Collector / Rule / Attribute Contract](../../AGENTS.md#collector--rule--attribute-contract) |
| `AsyncRule` | `{ ruleType: string; code: string; message: string; readonly async: true; decide(attrs: ReadonlyAttributes, signal: AbortSignal): Promise<RuleAnswer> }` — the same contract as `Rule` but for I/O, under a deadline (#225). `isAsyncRule` reads the `async` discriminant |
...
| `RuleAnswer` | `boolean \| RuleVerdict` — what `verify` / `decide` may answer. Read it with `ruleAnswerPassed(answer)`, never by truthiness: a failing verdict is an object |
| `RuleVerdict` | `{ passed: boolean; evaluation?: RuleEvaluation }` — the answer of a rule that fronts a policy evaluator. It is the return value because it is a fact about one invocation; see [docs/extending.md](../../docs/extending.md#reporting-the-evaluation-behind-an-answer) |
