...
- [`src/types.mts`](src/types.mts) — the decision contract.
  ...
  - `ReportRuleEvaluation` and `RuleEvaluation` (with `RuleEvaluationStatus`) — how a rule that fronts a policy evaluator reports the evaluation behind one answer: whether it ran, against which policy revision (`null` being the explicit unknown), and, for a completed one, which policies determined it (#199). `evaluate()` makes one reporter per invocation, and puts on it `boundDeterminingPolicies`, through which a rule names determining policies under this core's bounds — absent on a core that predates them, so a rule names none to one; a reported revision is held to `POLICY_REVISION_PATTERN` and `POLICY_REVISION_MAX_LENGTH`, and determining policies to `DETERMINING_POLICIES_MAX`, `POLICY_ID_MAX_LENGTH` and `POLICY_ID_FORBIDDEN_RANGES` — the check `isReportablePolicyId` makes, and the reporter's `boundDeterminingPolicies` builds a report that passes it. See [docs/extending.md](../../docs/extending.md#reporting-the-evaluation-behind-an-answer).
