...
- `values` は非空かつ同種の配列。`attrs.get(a)` が `values` と同じ型で、集合に含まれないときに通過します。欠落・型不一致・`NaN` は `false`（safe-deny）を返します — `NaN` はどの集合にも含まれませんが、同様に拒否します（#254）。`values` 内の重複要素は Rule の挙動に影響しません。
