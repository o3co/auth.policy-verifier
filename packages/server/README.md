...
`createVerifyRouter(config: VerifyRouterConfig): express.Router` — `VerifyRouterConfig` is defined, with a doc comment per field, in [`src/routes/verify.mts`](src/routes/verify.mts); that file is the reference. `createApp` builds it from `AppConfig`, passing the resolved `authenticator` rather than `jwt`. What each knob means:

- **Subject authentication — exactly one of `jwt` and `authenticator`** (#219); both, neither, or `null` for either is refused at construction.
  - `jwt` — the built-in bearer-JWT path, constructed by the router itself. Its type, `VerifyRouterJwtConfig` in [`src/jwt/tokenAuthenticator.mts`](src/jwt/tokenAuthenticator.mts), is discriminated on `validate`: `validate: true` carries `key` (from a `KeyResolverFactory`), `algorithms`, `issuer`, `audience`, optional `audienceClaim` (default `"aud"`) and `tokenType` (`"*"` pins nothing); `validate: false` requires `allowInsecureDecode: true` and is test-only. Both arms take `maxTokenAgeSeconds` (default 86400) and `clockToleranceSeconds` (0–300, default 0).
  - `authenticator` — an already-built `TokenAuthenticator`: what `createApp` hands in after resolving `oauth.authenticator`, and what a library consumer passes to run the router over a subject established some other way.
