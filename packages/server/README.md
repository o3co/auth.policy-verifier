...
**Role.** The Node-only host that turns [`@o3co/auth.policy-verifier.core`](../core/README.md)'s pipelines and evaluator into an HTTP decision endpoint. A composition root — the [standalone template](../../templates/standalone/README.md), or a project generated by [`create-app`](../../create-app/README.md) — calls `createApp` with its config and modules; the only workspace package the server depends on is core (the built-in collectors in [`@o3co/auth.policy-verifier.builtins`](../builtins/README.md) are passed in as a module, not imported).

**Owns:** the HTTP surface (`/verify`, `/verify/batch`, the liveness probe, `/metrics`) and its status codes and deny envelope; authenticating the subject (the built-in bearer-JWT authenticator, the key resolvers, and the registry through which a module supplies another authenticator); authenticating the calling service (`http.callerAuth`); the application config schema and the rule that both config boundaries read every knob the same way; running one decision and reporting it (the `decision` line, failure categories, Prometheus counters).

**Does not own:** evaluation semantics, the `Rule` / collector / `Module` contracts (core); any concrete collector, rule or resource parser (builtins, cedar, or the deployment's own modules); the process lifecycle — reading the config file, `app.listen`, signal handling — which stays with the composition root.
...
`createApp(options): Promise<express.Express>` — the options type is `CreateAppOptions` in [`src/app.mts`](src/app.mts), which is the reference; the knobs are:
...
- `config` — an `AppConfig` (see [AppConfigSchema](#appconfigschema--appconfig)). A config that never went through the schema is accepted and checked again at runtime by the same functions.
- `modules` — initialized in order with the server's `ServerModuleContext`, so both a plain core `Module` (collectors, rules, parsers) and a `Module<ServerModuleContext>` (key resolvers, token authenticators) are accepted.
- `logger` (optional) — structured logger for boot-time warnings and the verify router's failure and `decision` events; pino-compatible. Defaults to the console-backed logger at `config.logging.level`, so failures are never silent.
...
5. Mounts the metrics middleware app-wide (first, so it times everything), then, under `config.http.pathPrefix`: the liveness probe (`GET /_healthcheck`, the path every component of the stack answers on, with `GET /healthcheck` kept as a compatibility alias), the `/metrics` scrape endpoint, the optional caller-authentication gate, then `POST /verify` and `POST /verify/batch`.
6. Returns the configured `express.Express` instance.

### createVerifyRouter

`createVerifyRouter(config: VerifyRouterConfig): express.Router` — `VerifyRouterConfig` is defined, with a doc comment per field, in [`src/routes/verify.mts`](src/routes/verify.mts); that file is the reference. `createApp` builds it from `AppConfig`, passing the authenticator `oauth.authenticator` selects. What each knob means:

- `authenticator` — **required**: the built `TokenAuthenticator` the router runs to establish the subject (#219). The router builds no authenticator of its own (#259); the port is defined in [`src/auth/tokenAuthenticator.mts`](src/auth/tokenAuthenticator.mts). `createApp` hands in the one `oauth.authenticator` selects. Mounting the router yourself, either pass your own — a subject established some other way — or build the built-in bearer-JWT one with `createTokenAuthenticator(jwt, logger)`, handing it the same `logger` as the router so its `jwt_token_rejected` / `jwt_verification_unavailable` lines reach the same sink. `jwt` is a `VerifyRouterJwtConfig` ([`src/jwt/tokenAuthenticator.mts`](src/jwt/tokenAuthenticator.mts)), discriminated on `validate`: `validate: true` carries `key` (from a `KeyResolverFactory`), `algorithms`, `issuer`, `audience`, optional `audienceClaim` (default `"aud"`) and `tokenType` (`"*"` pins nothing); `validate: false` requires `allowInsecureDecode: true` and is test-only. Both arms take `maxTokenAgeSeconds` (default 86400) and `clockToleranceSeconds` (0–300, default 0). A config with no authenticator (`null` included), or still carrying the removed `jwt` option, is refused at construction, the latter naming this migration.
- `resourceParser`, `attributePipeline`, `rulePipeline` — required. The collector bounds (`collectorTimeoutMs`, `collectorDeadlineMs`, `collectorConcurrency`) belong to the pipelines, not to this config.
- `evaluateOptions` — evaluator semantics overrides; omitted means deny on an empty rule set. `ruleTimeoutMs`, `evaluateDeadlineMs` and `failures` are refused here at construction: the deadlines are this config's own fields, and the router keeps one failure record per decision (#200), so a `failures` handed in would be shared by every decision. A `signal` is combined with the caller's, never replaced by it.
- `maxBatchSize` (default 50) — most entries `POST /verify/batch` decides in one request.
- `batchConcurrency` (default 8) — how many of a batch's entries are decided at once (#183).
- `ruleTimeoutMs` (default 2000) / `evaluateDeadlineMs` (default 5000) — one asynchronous rule's budget, and all of a decision's asynchronous rules together (#225); overrun denies `rule_timeout`.
- Request limits (#118): `maxBodyBytes` (default 65536, the `limit` handed to `express.json()` and the outer envelope on a batch), `maxResourceLength` (default 512 characters), `maxActionLength` (default 64), `maxContextEntries` (default 64, every property and array element at every depth of `context`, which also bounds its depth) and `maxContextValueLength` (default 1024 characters, property names included).
- `logger` — sink for the router's failure events and the per-decision `decision` line (#111); defaults to the console-backed logger.
- `metrics` — optional `DecisionMetrics` seam (#111); omitted means decisions are logged but not counted. `createApp` wires the Prometheus implementation.
- `credentialToCollectors` — `"never"` (default) or `"expose"`: whether the raw credential reaches collectors as `CollectorContext.credential` (#175).
- `evaluationInResponse` — `"omit"` (default) or `"include"`: whether the response carries each rule's `evaluation` (#244); anything else is refused.

Every numeric knob admits the string form a HOCON env substitution produces and is read by `resolveBound` with the same bound `AppConfigSchema` applies (#157).
...
2. Hand the `Authorization` header to the authenticator. The built-in bearer-JWT one (`createTokenAuthenticator`) extracts `Bearer <token>` (the scheme matched case-insensitively) and returns 401 if it is missing or the scheme is not Bearer; any other authenticator answers with its own `code` / `message`, and steps 3–5 are its business.
3. If `validate` is `true`: verify the signature **and** the RFC 9068 §4 claims — `iss` against `issuer`, the audience claim (`aud`, or the claim `audienceClaim` names) against `audience`, and the `typ` header against `tokenType` (an `application/` prefix is ignored; `"*"` pins nothing). Returns 401 on failure. `createTokenAuthenticator` throws if any of the three is missing.
