...
import { checkTokenAuthenticatorSelection } from "./tokenAuthenticatorSelection.mjs";
	...
	oauth: z
		.object({
			...
			jwt: z
				 ...
				 * `transform` below. A deployment running another authenticator has
				 * no use for this block and is not made to write one.
				 */
				.optional(),
		})
		...
		.transform((data, ctx) => {
			...
			return { ...data, authenticator: selection.name };
		}),
	attribute: z.object({
